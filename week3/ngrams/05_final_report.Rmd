...
```{r load-counts}
year_counts <- read_tsv(here('./week3/ngrams/year_counts.tsv'), col_names = c('term','year','volume','bookcount'))

total_counts <- read.csv(here('./week3/ngrams/total_counts.csv'), col.names = c('year','total_volume','page_count','book_count'))
```
...
ngram_total <- left_join(year_counts, total_counts, by = 'year') %>% mutate(frequency = volume/total_volume)
```
...
![](ngram_viewer.png)
> Overall the shape of the graph is the same, however, it is missing those extra small up and down hill as the slope goes down for every term year.
...
>I would reevaluate the results of Michel et al. because it was stated "little use before that year, then a spike, then decay." However, looking at the graph in part d, it shows that after a decay, it gradually grows again. 
...
temp <- ngram_total %>%
  filter(year > term) %>%
  group_by(term) %>%
  summarize(peak = max(frequency))

peak <- inner_join(ngram_total, temp, by = c('frequency' = 'peak')) %>% filter(term.x <= year) %>% select(term = term.x, peak_year = year, volume, total_volume, peak = frequency)
```
...
half_peak <- left_join(temp2, ngram_total, by = 'term') %>% filter(half_peak >= frequency) %>% filter(term < year) %>% group_by(term) %>% select(term, peak_year, year, frequency) %>% filter(peak_year < year) %>% summarize(half = max(frequency), year = year) %>% filter(year == min(year))

half_life <- inner_join(peak, half_peak, by = 'term') %>% mutate(min_years = year - peak_year)
```
...
half_life %>%
  ggplot() +
  geom_point(aes(x = term, y = min_years)) +
  geom_point(aes(x = 1883, y = 7), colour = "blue") +
  geom_point(aes(x = 1910, y = 18), colour = "green") + 
  geom_point(aes(x = 1950, y = 11), colour = "red")
```
...
> There were one outlier, which was the year 1800. The year 1800 was forgotten more slowely, since it had a half_life of 23. It is a bit difficult to determine the pattern of the graph, however, it does seem that the as the term increases, it is more likyly one will foget particularly quickly. The values of the half-life seem to get closer as we move down the terms. 

```{r}
half_life %>%
  filter(min_years == min(min_years) | min_years == max(min_years))
```
